...
sed -i "s~dbms_template_path~$project_path~g" nginx/site.conf etc/systemd/gunicorn.service
sed -i "s~dbms_template_domain~$project_domain~g" nginx/site.conf src/config/settings.py

sudo ln -s $project_path/etc/nginx/site.conf /etc/nginx/sites-enabled/
