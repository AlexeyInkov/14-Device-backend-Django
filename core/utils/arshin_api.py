...
def request_to_arshin(reg_number: str, number: str) -> Response:
    ...
    msg = f"sleep {settings.TIMEOUT_BETWEEN_REQUEST_TO_ARSHIN} sec"
    logger.debug(msg)
    sleep(settings.TIMEOUT_BETWEEN_REQUEST_TO_ARSHIN)
    ...
    logger.debug(url)
    response = request("get", url, timeout=settings.TIMEOUT_FOR_REQUEST_TO_ARSHIN)
